...
try {
    ...
    & (Join-Path $ProjectRoot 'build.ps1') -Tests -ExecutionLockHeld
    Push-Location $ProjectRoot
    try {
        & java -Xms32m -Xmx256m -XX:+UseSerialGC -XX:ActiveProcessorCount=2 `
            -cp (Join-Path $ProjectRoot 'out') edu.academic.integrity.tests.AllTests
        if ($LASTEXITCODE -ne 0) {
            throw "Test runner exited with code $LASTEXITCODE"
        }
    } finally {
        Pop-Location
    }
